# Quick SDF Studio 0.5.0 クイックガイド

Quick SDF Studioは、法線から作られた顔影の下描きを、アーティストが顔を見ながら `Light` と `Shadow` で直すための専用環境です。左右反転、書き出し時の角度整合性調整、最終生成は自動で行われます。
...
1. [GitHub Releases](https://github.com/yeczrtu/QuickSDFBlender/releases/latest)から `quick_sdf_blender-0.5.0-windows-x64.zip` を取得します。
...
タイムライン上段のレールをドラッグすると、0～90°の補間結果を2D／3Dで連続確認できます。マウスを離すとプレイヘッドは最も近い既存キーへ吸着し、2D Canvas、3D表示、ペイント対象がそのキーへ揃います。ペイント開始時に別の角度へ戻ることはありません。通常のペイントからキーが自動追加されることもありません。

既定キー以外が必要な場合だけ、`Advanced` → `Angle Keys` → `Add Angle…` で角度を数値指定します。追加キーはその角度の法線ガイドから作られます。
...
一筆は選択中の角度キーだけへ即時反映されます。これにより、ソフトブラシ、低いStrength、筆圧を含むBlender標準Texture Paintの感触と応答速度を維持します。ほかの角度も直したいときは、タイムラインのサムネイルをクリックして同じように塗ります。

角度間に矛盾が残っていても、Export時に書き出し用コピーだけを自動調整します。元のCanvasは変更されないため、PropagateやValidateを押す必要はありません。一筆は選択中キーの1回のUndo／Redoとして扱われます。
...
3D Viewへ極端に近づいたときは、StudioがClip StartとNormal Falloffを一時調整して近距離ペイントを助けます。それでも投影できない場合は、少し引く、`Numpad5`でOrthographicへ切り替える、または2D Canvasで塗ってください。OccludeとBackfaceのユーザー設定は変更しません。
...
通常はチャンネル構成を変更する必要はありません。既定のlilToonパッキングは次の構成です。

| Channel | 黒（0） | 白（65535） |
|---|---|---|
| R | 右光スレッショルドの最小値 | 右光スレッショルドの最大値 |
| G | 左光スレッショルドの最小値 | 左光スレッショルドの最大値 |
| B | 顔SDFを使用 | 通常法線シェーディングへ戻す |
| A | 顔影を無効化 | 顔影を通常強度で使用 |
...
5. 通常どおり`Export Face Shadow Texture`を押します。
...
- `SDF Area`：対象Material SlotのUV領域が白、領域外が黒。白い場所で顔SDFを使います。
...
0.5.0はschema 5だけを扱います。以前のProjectを変換する処理はありません。対象Meshから0.5.0用の新しいProjectを作成してください。Native core ABIは0.4.0と同じ5です。
...
Studio Sessionはファイルへ保存しません。再読み込み後は必ず `Open Quick SDF Studio`から始まります。
