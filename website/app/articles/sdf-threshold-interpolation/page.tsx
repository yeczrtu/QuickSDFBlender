...
import { absoluteArticleUrl, basePath, getArticle, siteOrigin } from "../article-data";
...
export default function SdfThresholdInterpolationArticle() {
  const nearest = study.aggregate.nearestKey;
  const sdf = study.aggregate.sdfDistanceRatio;
  ...
  return (
    ...
    >
      <EvidenceNote>
        <p>512×512、7キー、6種類の数式形状を使う決定的な検証を新規作成しました。1～89度の89評価角度を正解画像と比較し、全画像の画素不一致率、IoU、時間方向の変化量を計測しています。掲載画像とJSONは同じスクリプトから生成されます。</p>
      </EvidenceNote>

      <h2 id="definition">入力と出力を定義する</h2>
      <p>光角度を0から1へ正規化し、7枚のキーを<code>t = 0, 1/6, 2/6 … 1</code>へ配置します。画素<code>p</code>におけるキー<code>i</code>の二値マスクを次のように定義します。</p>
      ...
      <p>シェーダーによって値や比較方向を反転して保存する場合はありますが、情報の意味は同じです。<code>u</code>は距離ではなく、角度方向の切替位置です。一般的なSDFが持つ境界からの長さや勾配は、完成テクスチャには残りません。</p>
      ...
      <pre><code>V(p) = (1 - f) M_i(p) + f M_i+1(p){"\n"}Light = V(p) &gt;= 0.5</code></pre>
      <p><code>M_i=0</code>、<code>M_i+1=1</code>の画素では、結果がLightになる条件は常に<code>f &gt;= 0.5</code>です。二値結果として評価する限り、変化する全画素が区間中央で同時に反転するため、今回の最近傍キーと同じ結果になりました。グレーのまま影色を混ぜればフェードになりますが、輪郭が移動する二値影とは別の表現です。</p>
      ...
      <p>各二値マスクからLight側を正、Shadow側を負とするSigned Distanceを作ります。隣接キーでShadowからLightへ変わる画素について、二つの境界までの距離から区間内の切替位置を求めます。</p>
      ...
      <ul>
        ...
        <li>距離変換：画素中心上のExact Euclidean Distance Transform</li>
        <li>評価：全画素不一致率、IoU、1度ごとの変化量</li>
      ...
      <dl className="metric-grid">
        <div><dt>最近傍の平均画素誤差</dt><dd>{percent(nearest.meanPixelErrorPercent)}</dd></div>
        <div><dt>SDFの平均画素誤差</dt><dd>{percent(sdf.meanPixelErrorPercent)}</dd></div>
        <div><dt>最近傍の平均IoU</dt><dd>{nearest.meanIoU.toFixed(3)}</dd></div>
        <div><dt>SDFの平均IoU</dt><dd>{sdf.meanIoU.toFixed(3)}</dd></div>
      </dl>
      ...
      <ArticleFigure
        ...
        height={1552}
      />
      <p>また、隣接する二枚ずつの補間は、キー位置で形がつながっても、区間境界で移動速度まで一致する保証がありません。位置は連続でも速度が滑らかとは限らない、C0連続の補間です。この制約は<a href="https://nagakagachi.hatenablog.com/entry/2024/03/02/140704">公開されているSDF距離比の解説</a>でも指摘されています。</p>
      ...
      <pre><code>M_i(p) &lt;= M_i+1(p)</code></pre>
