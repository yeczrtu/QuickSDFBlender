...
export default function BlenderWorkflowArticle() {
  return (
    <ArticleLayout
      article={article}
      toc={toc}
      lead="顔影制作は、角度ごとに別の影絵を完成させる作業ではありません。一つの影境界が、光の向きに応じて顔の上をどう移動するかを設計する作業です。法線ガイドを下描きに、直す順番と判断基準を説明します。"
    >
      <EvidenceNote>
        <p>Blender 5.1、Quick SDF Paint 0.7.1、1024px、0～90度の既定8キー、左右ミラーONで確認しました。操作の列挙ではなく、設定異常を先に見分ける方法、大形から直す順番、顔の部位ごとの判断、静止画と動きの評価を中心にしています。</p>
      </EvidenceNote>
      ...
      <p>方針が曖昧なまま各キーを個別に仕上げると、停止画ではきれいでも、角度を動かしたときに影の設計が途中で変わって見えます。</p>
      ...
      <p>正面方向が違う場合は、顔を正面から見て<code>Use This View as Front</code>を使います。全体の影量だけが多い、または少ない場合は<code>Shadow Amount</code>を調整します。PoseやMeshを変更した場合は<code>Rebake Base</code>を使います。これらは手描き範囲を保持して下描きだけを更新します。</p>
      <ArticleFigure
      ...
      <ol>
        <li>0度と90度で変化の両端を確認する</li>
        <li>中央付近で、顔影の主役となるシルエットを作る</li>
      ...
      <p>キーをクリックするとその位置が編集対象になります。キーの間へTimelineを動かすと補間状態を確認でき、そこで実際に画素を変えた場合だけ新しいキーが作られます。確認だけで画像が増えることはありません。</p>
      <ArticleFigure
      ...
      <p>対象外の面がMaterial Slotへ含まれている可能性があります。顔の肌だけに分離できない場合は、Quick SDF Paintの補助マスク<code>SDF Area</code>で角度別顔影を使う範囲を明示します。</p>
      <h3>左右で明暗の進行が逆</h3>
