...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    images: [{
      ...
      width: 1200,
      height: 630,
      alt: "角度別マスク、符号付き距離場（SDF）、切替位置という生成段階と7つの入力キー",
    }],
  },
  twitter: {
...
const toc = [
  ...
  { id: "stored-value", label: "保存するのは切替位置" },
  ...
  return (
    ...
    >
      <EvidenceNote title="本記事の整理方法">
        <p>複数の公開実装を、角度別マスクの作成、境界補間、切替位置への圧縮、ランタイム評価という共通のデータフローに整理しました。さらに、法線編集、固定マスク、複数画像の切替との違いを、アーティストが方式を選ぶための比較表にまとめています。</p>
      </EvidenceNote>

      <h2 id="why">なぜ顔だけ別の陰影制御が必要になるのか</h2>
      <p>一般的なセルシェーディングでは、表面法線<code>N</code>と正規化したライト方向ベクトル<code>L</code>の内積<code>N·L</code>を計算し、一定値を境に明部と影部を切り替えます。形状に自然に追従するため、身体や衣服には扱いやすい方法です。</p>
      ...
      <h2 id="stored-value">保存するのは「影の濃さ」ではなく「切替位置」</h2>
      <p>ここでは4つの量を区別します。<code>L</code>は頭部のローカル座標で表した正規化ライト方向ベクトルです。<code>φ</code>は、<code>L</code>の前後・左右成分から求める水平方向の実ライト角です。</p>
      <p><code>t</code>は、制作で定めた<code>Light Starts</code>から<code>Full Light</code>までを0～1に正規化した制作進行度です。<code>u(p)</code>は、UV画素<code>p</code>が明部へ切り替わる正規化位置を表します。画面に表示する0°～90°は<code>t</code>を操作しやすくするための制作上の目盛りで、物理的なライト角<code>φ</code>そのものではありません。</p>
      <p>以下では、明部をLight状態、影部をShadow状態と呼びます。顔のUV上に画素A、B、Cがあり、<code>t</code>を0から1へ進める場面を考えます。Aは早くLight状態になり、Bは途中で、Cは<code>Full Light</code>の近くまでShadow状態に残るとします。完成テクスチャには、3つの切替位置<code>u(A)</code>、<code>u(B)</code>、<code>u(C)</code>を保存します。</p>
      ...
      <div className="process-flow" role="list" aria-label="顔影スレッショルドマップの処理段階">
        <div role="listitem"><span>01 制作（Authoring）</span><strong>角度別マスクで意図を定義</strong></div>
        <div role="listitem"><span>02 補間（Interpolation）</span><strong>離れた境界の間を補間</strong></div>
        <div role="listitem"><span>03 符号化（Encoding）</span><strong>画素ごとの切替位置へ圧縮</strong></div>
        <div role="listitem"><span>04 評価（Evaluation）</span><strong>制作進行度と比較</strong></div>
      </div>
      <ArticleFigure
        src={`${research}threshold-map-overview-card.png`}
        alt="角度別の二値マスクから画素ごとの切替位置を作り、任意の進行度で顔影を再構成する流れ"
        caption="角度別マスクは制作入力、中央のグレースケールは切替位置、右側は任意の進行度から再構成した二値影。符号付き距離場は、入力から切替位置を求める途中でだけ使う。"
        width={1200}
        height={630}
      />

      <h3>1. 制作（Authoring）：代表キーで望む影を決める</h3>
      <p>制作進行度<code>t</code>上に代表キーをいくつか置き、各キーで<code>Light</code>にしたい領域を白、<code>Shadow</code>にしたい領域を黒として定義します。法線から下描きを作っても、最初から手描きしても構いません。大切なのは、自動計算を完成結果とせず、アーティストが輪郭を直接修正できることです。</p>
      <p>必要なキー数に普遍的な正解はありません。<a href="https://cgworld.jp/article/202306-hifirush01.html">『Hi-Fi RUSH』の公開制作記事</a>ではDCC上でライトを等間隔に回し、複数の二値画像を生成・修正する工程が示されています。一方、<a href="https://erichu33.github.io/ASPDocs/en/articles/face-shadow-map-creation-and-baking-workflow.html">Anime Shading Plusの公開手順</a>では9枚を例にしています。</p>
      <p>キーを増やせば局所的な制御はしやすくなりますが、制作量と不整合の可能性も増えます。粗い間隔から始め、境界の経路を変えたい区間にだけ追加する方が管理しやすくなります。</p>

      <h3>2. 補間（Interpolation）：白黒ではなく境界の移動を補う</h3>
      <p>角度キーを順番に切り替えるだけでは、影は段階的に飛びます。画像のRGB値を線形に混ぜてもグレーは作れますが、二値化した結果では差分領域が途中で一斉に反転し、輪郭が滑らかに進まない場合があります。</p>
      <p>符号付き距離場（Signed Distance Field、SDF）を使う方法では、各マスクについて境界までの距離を求めます。隣接する2つの境界からの距離比によって、その画素が区間内のどこで切り替わるかを推定します。<a href="https://nagakagachi.hatenablog.com/entry/2024/03/02/140704">SDF Based Transition Blending for Shadow Threshold Map</a>と<a href="https://github.com/akasaki1211/sdf_shadow_threshold_map">公開ツール実装</a>で、この考え方を確認できます。</p>
      <pre><code>{`transition = distanceFromA / (distanceFromA + distanceFromB)`}</code></pre>
      <p>SDFは有効な補間方法ですが、このデータ表現の必須条件ではありません。入力が単純なら累積マスクなど別方式でも切替位置を作れます。SDFが担当するのは、離れた輪郭の途中位置を距離から求める部分です。</p>
      <p>距離比の式、4方式の再現可能な比較、トポロジー変化での限界は、<a href={articlePath("sdf-threshold-interpolation")}>角度別マスクを1枚にまとめる方法：SDF距離補間の比較</a>で詳しく扱います。</p>

      <h3>3. 符号化（Encoding）：状態列を1つの値へ圧縮する</h3>
      <p>角度別マスクを<code>M(p, t)</code>とします。ある画素が制作進行度に沿って1回だけ<code>Shadow</code>から<code>Light</code>へ変わるなら、その履歴は1つの切替位置<code>u(p)</code>で表せます。</p>
      <pre><code>{`M(p, t) = [ t ≥ u(p) ]`}</code></pre>
      <p>複数の画像をランタイムへ持ち込まず、「いつ変わるか」だけを1枚に残せるのはこのためです。左右を別チャンネルに保存する、左右対称ならUVを反転する、といった格納方法はシェーダーごとに異なります。</p>

      <h3>4. 評価（Evaluation）：頭部に対する光方向と比較する</h3>
      <p>シェーダーは頭部のローカル座標でライト方向ベクトル<code>L</code>を求め、顔の<code>Forward</code>と<code>Right</code>を基準に実ライト角<code>φ</code>を計算します。さらに<code>φ</code>を制作進行度<code>t</code>へ写像し、左右のどちらから光が来るかに応じてチャンネルまたはUV方向を選びます。最後に、<code>t</code>とテクスチャの切替位置<code>u(p)</code>を比較します。</p>
      <p>頭部のローカル座標を使うことで、キャラクターや頭が回転しても、顔に対するライトの相対方向を保てます。</p>

      <h2 id="comparison">ほかの顔影制御とどう使い分けるか</h2>
      <p>方式によって、アーティストが直接編集する対象が異なります。以下は複数の方式を同じ評価軸で比較した、本記事独自の整理です。</p>
      <table>
        ...
        <tbody>
          ...
          <tr><th scope="row">スレッショルドマップ</th><td>画素ごとの切替位置</td><td>描いた輪郭の変化を1枚にまとめられる</td><td>1画素につき基本的に1回の切替しか表せない</td></tr>
        </tbody>
      </table>
      <p>身体や衣服の陰影まで置き換える必要はありません。形状に沿った反応が望ましい部分には法線を使い、輪郭を設計したい顔だけにスレッショルドマップを適用する構成が現実的です。</p>

      <h2 id="monotonic">1画素1回の切替という制約</h2>
      <div className="state-sequence">
        <div><span>1つの閾値で表せる</span><strong>1方向の状態列</strong><code>S S S L L L L</code></div>
        <div><span>1つの閾値では表せない</span><strong>往復する状態列</strong><code>S L S L L S L</code></div>
      </div>
      <p>一度<code>Light</code>になった画素が後のキーで<code>Shadow</code>へ戻る場合、その履歴を1つの切替位置へ無損失に圧縮できません。2回以上の切替が必要なら、追加チャンネル、複数のマップ、二次元の光方向データなど、表現自体を拡張する必要があります。</p>
      <p>そのため角度別マスクは、独立した完成イラストの集まりではなく、1方向へ変化する同じ影境界として設計します。制作ツールの<code>Monotonic Guard</code>や書き出し時の修復は、この表現上の制約を守るための処理です。</p>

      <h2 id="suitable">この方式が適している条件</h2>
      <ul>
        <li>物理的な正しさより、顔影を図形として整えて見せたい</li>
        <li>主に1つの代表ライト方向を使う</li>
        <li>頭部を基準にした左右方向の変化が重要</li>
        <li>制作進行度に沿って明部が1方向へ増減する</li>
        <li>顔に安定したUVがある</li>
        <li>アーティストが法線より白黒の輪郭を直接修正したい</li>
        <li>ランタイムで多数の角度画像を保持したくない</li>
      </ul>
      <p>特に「各代表キーで完成形を描けること」と「ランタイムでは1枚として扱えること」の間をつなぐ点が特徴です。</p>

      <h2 id="limitations">1枚のスレッショルドマップだけでは扱えないもの</h2>
      <h3>上下方向の大きなライト移動</h3>
      <p>水平方向だけを1つの値に圧縮した実装では、上下方向を独立して表現できません。上下にもライトが大きく動く場合は、別パラメーター、複数のマップ、通常の法線陰影との合成が必要です。</p>
      <h3>髪や手が落とす幾何学的な影</h3>
      <p>遮蔽物を見ていないため、髪が額へ落とす影や手が顔を横切る影は、リアルタイムのシャドウマップなど別の可視性計算で扱います。マップに鼻影らしい図形を描くことはできますが、それは物理的な投影ではなく、実ライト角<code>φ</code>に関連付けたデザインです。</p>
      <h3>複数ライトと近距離のポイントライト</h3>
      <p>1つの代表方向を前提とする方式では、複数方向からの寄与を1つの閾値だけで表せません。近距離のポイントライトでは顔の場所ごとにライト方向も変わるため、頭部中心からの1方向近似には限界があります。</p>
      <h3>極端な表情変形</h3>
      <p>UVベースなので通常の表情変形には追従しますが、描いた境界自体もメッシュと一緒に伸びます。口を大きく開く表情などで対応できない場合は、表情別の補正や適用範囲マスクを検討します。</p>

      <h2 id="decisions">制作前に決める6項目</h2>
      <ol>
        <li><strong>顔の座標系：</strong><code>Forward</code>、<code>Right</code>、<code>Up</code>を明示する。</li>
        <li><strong>角度の意味：</strong>実ライト角<code>φ</code>から制作進行度<code>t</code>への写像、値が増える方向、白黒の意味を決める。</li>
        <li><strong>左右の扱い：</strong>UV反転で共有するか、左右を別データとして持つか決める。</li>
        <li><strong>キー密度：</strong>枚数を均等に増やさず、境界の経路が変わる区間へ置く。</li>
        <li><strong>1方向の変化：</strong>各画素が制作進行度に沿って1回だけ切り替わる状態を保つ。</li>
        <li><strong>出力仕様：</strong>チャンネル、反転、ビット深度、色空間を記録する。</li>
      </ol>
      <p>この6項目が曖昧だと、影形状自体が正しくても、シェーダー上では左右反転、進行度のずれ、階調劣化として現れます。</p>
      ...
      <table>
        ...
        <tbody>
          <tr><th scope="row">Hi-Fi RUSH GDC講演</th><td>threshold map</td><td>ライト方向から求めた値と比較する完成テクスチャ</td></tr>
          <tr><th scope="row">CGWORLD</th><td>Face Threshold Map</td><td>Hi-Fi RUSHの顔影制御</td></tr>
          <tr><th scope="row">ながむしメモ</th><td>Shadow Threshold Map</td><td>SDF距離比で生成する切替位置マップ</td></tr>
      ...
      <p>顔影スレッショルドマップは、完成した顔影の色を保存する画像ではありません。各画素が制作進行度<code>t</code>のどこで<code>Light</code>と<code>Shadow</code>を切り替えるかを、切替位置<code>u(p)</code>として保存するデータです。</p>
      <ol>
        <li>代表キーごとに望ましい白黒マスクを作る</li>
        <li>離散した境界の移動を補間する</li>
        <li>各画素の切替位置を1つの値へ変換する</li>
