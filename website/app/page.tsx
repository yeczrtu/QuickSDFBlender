...
const copy = {
  ja: {
    ...
    basics: [
      ...
      ["1回のペイント", "青枠で選ばれている1枚だけを直します。ほかのキーは変わりません。"],
    ...
    step4Body: "画面下の8つのサムネイルが、実際に塗れる影のキーです。サムネイルをクリックすると、その1枚が編集対象になります。プレイヘッドをドラッグすると、キーとキーの間も含めて影の変化を連続して確認できます。",
    step4Actions: [
      "プレイヘッドを左右へドラッグする",
      "2Dと3Dで、明るい部分が広がる様子を確認する",
      "直したい位置でマウスを離す",
      "必要な部分だけStep 3と同じように修正する",
    ],
    step4Note: "既定キーは0.0、12.9、25.7、38.6、51.4、64.3、77.1、90.0°の8つです。45°のSideは中央位置を示す目印で、追加の画像ではありません。プレイヘッドを離すと最も近いキーが選ばれ、そのままペイントできます。ドラッグしても画像は増えません。",
    angleTerms: [
      ["0° → 90°", "ライトの実角度ではありません。0°はLight Starts、45°はSideの目印、90°はFull Lightです。"],
      ["サムネイル／プレイヘッド", "サムネイルは実際に塗る8枚、プレイヘッドはその間の見え方まで確認するためのものです。"],
    ],
    gifAlt: "プレイヘッドをLight StartsからFull Lightへ動かすと、2Dマスクと3Dの顔影が変化するQuick SDF Studioの画面",
    step4Legend: [
      ["1", "ドラッグ中：キーの間を含めて影の変化を確認"],
      ["2", "マウスを離したあと：最も近いキーをそのまま編集"],
    ] as LegendItem[],
    step5Label: "STEP 5",
    ...
    outputBody: "完成するのは、lilToon／liltoonUE用の16-bit RGBA PNGが1枚です。UnityやUnreal Engineへ読み込むときはsRGBを無効にし、色ではなくデータとして扱います。",
    ...
    footer: "Quick SDF Studio v0.4.0 — Blender 5.1／Windows x64",
    imageAlts: {
  ...
  en: {
    ...
    basics: [["Light / Shadow", "Light paints white; Shadow paints black."], ["0° → 90°", "This is not the light’s physical angle. The lit area grows from Light Starts at 0°, through the Side reference at 45°, to Full Light at 90°."], ["One stroke", "A stroke changes only the blue-outlined key. The other keys stay untouched."], ["Export", "Quick SDF fixes continuity across all eight guides without changing the images you painted."]],
    ...
    step4Body: "The eight thumbnails along the bottom are the shadow keys you can paint. Click a thumbnail to edit that image. Drag the playhead to preview the continuous transition between keys in both 2D and 3D.",
    step4Actions: ["Drag the playhead left or right", "Watch the lit area spread in 2D and 3D", "Release the mouse where you want to make a correction", "Paint the selected key as in Step 3"],
    step4Note: "The eight default keys are 0.0, 12.9, 25.7, 38.6, 51.4, 64.3, 77.1, and 90.0°. The Side marker at 45° is a midpoint reference, not another image. Releasing the playhead selects the nearest key so you can paint immediately. Dragging never creates extra images.",
    angleTerms: [["0° → 90°", "This is not the light’s physical angle. 0° is Light Starts, 45° marks Side, and 90° is Full Light."], ["Thumbnails / playhead", "The thumbnails are the eight images you can paint. The playhead previews how the shadow looks between them."]],
    gifAlt: "The Quick SDF playhead moves from Light Starts to Full Light while the 2D mask and 3D face shadow change",
    step4Legend: [["1", "While dragging: preview the transition between keys"], ["2", "After release: edit the nearest key immediately"]] as LegendItem[],
    step5Label: "STEP 5",
    ...
    outputBody: "The finished asset is one 16-bit RGBA PNG for lilToon or liltoonUE. Disable sRGB when importing it into Unity or Unreal Engine so it is treated as data rather than color.",
    outputSpecTitle: "View the output texture specification",
    outputRows: [["Format", "One 16-bit RGBA PNG"], ["R", "When light comes from the character’s right, the point where the pixel becomes Light"], ["G", "When light comes from the character’s left, the point where the pixel becomes Light"], ["B", "0"], ["A", "65535 (opaque)"], ["R / G values", "Larger values become Light earlier. 0 stays Shadow through the end; 65535 is Light from the start."]],
